...
import { ST3Query, GroupBy, addtionalData, PoPInfo, PoPList, Granularity } from 'types';
...
interface queryFilter {
    name: string;
    value: Array<SelectableValue>;
    id: string;
    customOptions?: SelectableValue<string>[];
}

interface State {
    metrics: SelectableValue<any>[] | any[];
    filter?: queryFilter[];
    platforms: SelectableValue<any>[] | any[];
    groupBy: GroupBy;
...
export class CommonQueryField extends React.Component<Props, State> {
    constructor(props: Props) {
        ...
        const f = statusFilter.map((v) => { return v.value });
        console.log(options.metrics,options.metrics?.split(','),options.metrics?.split(',').length)
        this.state = {
            metrics: options.metrics?.split(',') || [],
            filter: Object.entries(options.filters).filter(v => f.includes(v[0])).map((v) => { return { name: v[0], value: v[1].split(','), id: uniqueId() } }),
            ...
            type: "transfer",
            accountHash: {label:'default: '+ds.dsSettings.jsonData.defaultAccountHash,value:ds.dsSettings.jsonData.defaultAccountHash},
    ...
    updateOptions = () => {
        ...
        let filters = {};
        Object.assign(filters,...(this.state.filter?.map((v)=>{return {[v.name]:v.value.map(v=>v.value).join(',')}})));
        onChange({
            ...options,
    ...
    render() {
        ...
        const loadOptions = (query: string,type: string='pops') => {
            ...
            const { ds } = this.props;
            return new Promise<Array<SelectableValue<string>>>(resolve => resolve(
                type=='pops'?ds.get('/api/v1/pops').then(res=>{
                    return res.list.map((p: PoPInfo)=>{
                        return {label: p.country+': '+p.code,value: p.code}
                    })
                }):(type=='accounts' || type=='hosts' ? ds.get('/api/v1/accounts/'+this.state.accountHash.value+'/search?search='+query).then(res=>{
                    if(type=='hosts'){
                        return res['hostnames'].map((host: {hostHash: string;name: String;accountHash: string;accountName: string;scopeId: string})=>{
                            return {label: host.name+'('+host.hostHash+(host.accountHash!=this.state.accountHash.value?' below to account'+host.accountHash:'')+')',value: host.hostHash}
                        })
                    }else if(type=='accounts'){
                        console.log('asd','load account',res['accounts']);
                        return res['accounts'].map((account: {accountHash: string;accountName: string;displayName: string;grandparent:string;parent:string; id:string})=>{
                            return {label: account.accountHash+'('+account.accountName+' => '+account.parent+')',value:account.accountHash}
                        })
                    }else{
                        return 
                    }
                    ...
                    //     })
                    // })
                }):(type=='billingRegions'?[{label:'North America + Europe',value:'us'},{label:'Asia',value:'as'},{label:'South America',value:'sa'},{label:'Oceania',value:'oc'}]:[])))
            )
        ...
        return (
            ...
            <div style={{marginBottom:4}}>
                <HorizontalGroup>
                    <label className={"gf-form-label width-6 query-keyword"} style={{marginRight:0}}>Metrics Type</label>
                    <Select
                    options={[{label:"Print transfer statics",value:"transfer"},{label:"Print statusCode statics",value:"status"},{label:"Print cachedObject statics",value:"storage"}]}
                    onChange={(v)=>onSingleValueChange({type: v.value})}
                    defaultValue={"transfer"}
                    value={this.state.type}
                    />
                    <label className={"gf-form-label width-6 query-keyword"} style={{marginRight:0}}>For Account</label>
                    <AsyncSelect
                ...
                <div style={{ marginBottom: 4 }}>
                    <HorizontalGroup>
                        <label className={"gf-form-label width-6 query-keyword"} style={{ marginRight: 0 }}>Metrics</label>
                        <MultiSelect
                            options={selectOptions.Metrics}
                            onChange={(v) => onSingleValueChange({ metrics: v.map((v) => { return v.value }) })}
                ...
                <div style={{ marginBottom: 4 }}>
                    <HorizontalGroup>
                        <label className={"gf-form-label width-6 query-keyword"} style={{ marginRight: 0 }}>*Platforms</label>
                        <MultiSelect
                            options={this.state.platforms.find((v) => { return ["DELIVERY", "INGEST", "SHIELDING"].includes(v) }) ? platformsDef.slice(0, 3) : (this.state.platforms.length ? platformsDef.slice(3) : platformsDef)}
                            onChange={(v) => { onSingleValueChange({ platforms: v.map((v) => { return v.value }) }) }}
                            width={25}
                ...
                <div style={{ marginBottom: 4 }}>
                    <HorizontalGroup>
                        <label className={"gf-form-label width-6 query-keyword"} style={{ marginRight: 0 }}>GroupBy</label>
                        <Select
                            options={selectOptions.GroupBy}
                            onChange={(v) => { onSingleValueChange({ groupBy: v.value }) }}
                            width={25}
                            value={this.state.groupBy}
                        />
                    </HorizontalGroup>
                </div>
                <div style={{ marginBottom: 4 }}><HorizontalGroup>
                    <label className={"gf-form-label width-6 query-keyword"} style={{ marginRight: 0 }}>Granularity</label>
                    <Select
                        options={[{ value: "PT5M", label: "5 Minutes" }, { value: "PT1H", label: "1 Hour" }, { value: "P1D", label: "1 Day" }, { value: "P1M", label: "1 Month" }]}
                        onChange={(v) => { onSingleValueChange({ interval: v.value }) }}
                        width={25}
                ...
                {this.state.filter?.map((f) => (
                    ...
                    <div style={{ marginBottom: 4, width: "100%" }}>
                        <HorizontalGroup>
                            ...
                            <Select
                                width={25}
                                options={selectOptions.Filter.filter(v => (!this.state.filter?.map(fv => fv.name).includes(v.value)) || v.value == f.name)}
                                onChange={(v) => { onFilterChange({ id: f.id, name: v.value || "empty", value: [] }) }}
                                value={f.name}
                            />
                            <AsyncMultiSelect
                                ...
                                width={42}
                                defaultOptions
                                onChange={(v) => { onFilterChange({ id: f.id, name: f.name, value: v }) }}
                                allowCustomValue
                            ...
                            />
                            <Button variant="secondary" size="xs" onClick={(e) => { onFilterRemove(f.id) }}>
                                <Icon name="trash-alt" />
                            </Button>
                        </HorizontalGroup>
                    </div>
