...
export class ST3DataSource extends DataSourceApi<ST3Query, ST3DataSourceOptions> {
  ...
  query(options: DataQueryRequest<ST3Query>): Observable<DataQueryResponse> {
    ...
    console.log(
      getTemplateSrv()
        .getVariables()
        .map(v => {
          console.log(v, v.label, v.name, v.type, v.toString());
        })
    );
    ...
    const streams = options.targets.map(target => {
      ...
      if (ExploreMode.Logs === options.exploreMode) {
        return this.log(query).then((res: any) => {
          return { data: [res], key: target.refId };
        });
      } else {
        return this.analytics(query, options.scopedVars).then((res: any) => {
          return { data: [res], key: target.refId };
        });
      }
    });
  ...
  log(options: ST3Query) {
    return this.get(
      '/api/v1/accounts/' +
        (options.accountHash || this.dsSettings.jsonData.defaultAccountHash) +
        '/analytics/' +
        options.type +
        '?' +
      ...
      let columnIndexs = Object.entries(metricsData[0].metrics)
        .filter(v => metrics.includes(v[1]))
        .map(v => parseInt(v[0], 10)); //metricsData[0].metrics.filter(v=> metrics.includes(v) ).map((v,i)=>i);
      ...
      dataFields.push({
        name: 'usageTime',
        values: metricsData[0].data.map((v: number[]) => {
          return v[0];
        }),
        type: FieldType.time,
      });
